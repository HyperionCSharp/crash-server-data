...
local function setupPlayer(playerIdx)
    ...
    -- add state bag field
    if Player then
        Player(playerIdx).state['crash/playerData@id'] = playerId
    end
...
local function getExportEventName(resource, name)
	return string.format('__crash_export_%s_%s', resource, name)
end

function AddExport(name, fn)
    if not Citizen.Traits or not Citizen.Traits.ProvidesExports then
        AddEventHandler(getExportEventName('crash/playerData.v1alpha1', name), function(setCB)
            setCB(fn)
        end)
    end
